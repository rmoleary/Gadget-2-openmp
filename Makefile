...
CC       =  mpicc  -profile-functions -profile-loops=all -profile-loops-report=2             # sets the C-compiler
OPTIMIZE =  -O3 -fast -align -ipo #-Wall         # sets optimization and warning flags
MPICHLIB =  -lmpich -static
...
ifeq ($(SYSTYPE),"MPA")
CC       =  mpicc -std=gnu99 -g -openmp #-profile-loops=all #-g #-prof-use  # -profile-functions -profile-loops=all -profile-loops-report=2  -std=gnu99   #-std=c99
...
FFTW_LIBS= -L/usr/local/lib #-static
MPICHLIB =
HDF5INCL =  
HDF5LIB  =  #-lhdf5 -lz 
endif
...
LIBS   =   $(HDF5LIB)   $(MPICHLIB)  $(GSL_LIBS) -lgsl -lgslcblas -lm $(FFTW_LIB) 
